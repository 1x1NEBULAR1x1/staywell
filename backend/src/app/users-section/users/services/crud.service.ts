...
export class CrudService {
  ...
  async findOne(
    ...
    if (!user && !check_only) throw new NotFoundException('User not found');
  ...
  async update({
    ...
    if (!where || Object.keys(where).length === 0)
      throw new BadRequestException(
        'No parameters provided for user search for update',
      );
    ...
    if (auth.role !== Role.ADMIN && auth.id !== where.id)
      throw new ForbiddenException(
        'You are not allowed to make this operation',
      );

    const image = file
      ? this.filesService.saveImage({ file, dir_name: 'USERS' })
      : data.image;
    ...
    if (update_data.hashed_password) {
      update_data.hashed_password = await this.hashPassword(
