...
export class ListService {
   ...
   */
  async findAll(
    filters: ApartmentBedsFiltersDto,
  ): Promise<BaseListResult<ExtendedApartmentBed>> {
    const query_options = this.prisma.buildQuery<ApartmentBed>({
      filters,
      customFilters: this.customFilters,
    });
    const { items, total } =
      await this.prisma.findWithPagination<ExtendedApartmentBed>({
