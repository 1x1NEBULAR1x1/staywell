 ...
 */
export class BaseFiltersDto<T extends { id: string }> implements BaseFiltersOptions<T> {
  @ToInt({
    required: false,
    min: 0,
  ...
  @ToEnum({ enum: SortDirection, required: false, description: "Sort direction", example: SortDirection.desc, default: SortDirection.desc })
  ...
  @ToString({ required: false, description: "Search", example: "Search by name", max: 8192 })
  search?: string;
}
