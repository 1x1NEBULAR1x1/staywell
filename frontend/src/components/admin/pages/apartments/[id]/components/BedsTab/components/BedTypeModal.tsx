import type { BedType, CruddableTypes } from "@shared/src";
...
type FormData = CruddableTypes<"BED_TYPE">["create"] & {
  image_type: "file" | "url";
  files: File[];
  url: string;
};
...
export const BedTypeModal = ({
  onClose,
  editing_bed_type,
}: BedTypeModalProps) => {
  const { refetch: refetch_bed_types } = useModel("BED_TYPE").get({
    skip: 0,
    take: 1000,
  });
  const toast = useToast();
  const mutation = editing_bed_type
    ? useModel("BED_TYPE").update(editing_bed_type.id)
    : useModel("BED_TYPE").create();
  ...
  const handleSubmit = async (data: FormData) => {
    try {
      ...
      toast.success(
        `Bed type ${!editing_bed_type ? "created" : "updated"} successfully`,
      );
      onClose();
    } catch (error) {
      isAxiosError(error) &&
        toast.error(
          `Error during ${!editing_bed_type ? "creation" : "update"}: ${error.message}`,
        );
      console.error(error);
    } finally {
      refetch_bed_types();
