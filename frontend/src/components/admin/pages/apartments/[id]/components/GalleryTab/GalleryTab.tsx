...
export const GalleryTab = ({ apartment }: { apartment: ExtendedApartment }) => {
  const excludedCount = apartment.images.filter(
    (img) => img.is_excluded,
  ).length;
  const activeCount =
    apartment.images.filter((img) => !img.is_excluded).length +
    (apartment.image ? 1 : 0);
      ...
      <div className={classes.header}>
        <div className={classes.header_content}>
          ...
          <p className={classes.subtitle}>
            Manage apartment images. Set main image, add new photos, or delete
            existing ones.
          </p>
        </div>
        <div className={classes.stats}>
          <div className={classes.stat_item}>
            <span className={classes.stat_value}>
              {apartment.images.length + (apartment.image ? 1 : 0)}
            </span>
