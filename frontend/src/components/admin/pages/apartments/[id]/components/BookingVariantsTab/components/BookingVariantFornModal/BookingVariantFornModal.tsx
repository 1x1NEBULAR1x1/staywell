...
export const BookingVariantFornModal = ({
  onClose,
}: BookingVariantFornModalProps) => {
  ...
  const { data: apartments, isLoading: is_apartments_loading } = useModel(
    "APARTMENT",
  ...
  const handleCreate = async (
    data: CruddableTypes<"BOOKING_VARIANT">["create"],
  ) => {
    try {
        ...
        <SelectField
          ...
          options={
            apartments?.items.map((apartment) => ({
              label: apartment.name || `Apartment #${apartment.number}`,
              value: apartment.id,
            })) || []
          }
        />
