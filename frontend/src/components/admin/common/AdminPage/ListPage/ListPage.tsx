...
import { CreateButton, Filters, List } from "./components";
import type { FiltersConfig } from "./components/Filters/types";
import type { ColumnConfig } from "./components/List/components";
    ...
    if (filters.sort_field === field) {
      const new_direction =
        filters.sort_direction === SortDirection.asc
          ? SortDirection.desc
          : SortDirection.asc;
      setFilters({ sort_direction: new_direction } as Partial<
        GettableTypes<M>["filters"]
      >);
    } else {
  ...
  return (
    <AdminPage title={formatToTitle(GETTABLE_DATA[model])}>
      <Filters<M>
        ...
        filters_config={
          typeof filters_config === "function"
            ? filters_config()
            : filters_config
        }
        ...
        create_button={
          create_modal && (
            <CreateButton
              label={`Create ${model.toLowerCase()}`}
      ...
      <List<GettableTypes<M>["model"]>
        ...
        current_sort_field={
          filters.sort_field as keyof GettableTypes<M>["model"]
        }
      ...
      {is_modal_open &&
        create_modal &&
        cloneElement(create_modal, { onClose: () => setIsModalOpen(false) })}
    </AdminPage>
  );
};
