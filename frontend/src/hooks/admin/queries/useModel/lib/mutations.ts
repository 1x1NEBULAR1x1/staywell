...
import { type CrudApi, query_client } from "@/lib/api";
import { QUERY_KEYS } from "./query-keys";
import type { CachedList, MutationContext } from "./types";
import {
  handleMutationError,
 ...
 */
export function initUpdateMutation<M extends CRUDDABLE_NAMES>({
  api,
  model,
  id,
}: {
  ...
  >({
    ...
    onMutate: async (new_data) => {
      ...
      optimisticUpdateCache<M, CruddableTypes<M>["model"]>({
        ...
        id,
        updateFn: (old) =>
          !old ? old : { ...old, data: { ...old.data, ...new_data } },
      });

      return context;
    },
    onError: (_error, _data, context) =>
      handleMutationError({ context, model, id }),
  });
}
/**
 * Create mutation factory for creating items
 */
export function initCreateMutation<M extends CRUDDABLE_NAMES>({
  api,
  model,
}: {
  api: CrudApi<M>;
  ...
  >({
    ...
    onSuccess: (response) => {
      ...
      query_client.setQueryData(QUERY_KEYS(model).find(new_item.id), {
        data: new_item,
      });
      ...
      query_client.setQueriesData(
        ...
        (old: CachedList<CruddableTypes<M>["model"]>) =>
          !old?.data?.items
            ...
            : {
                ...old,
                data: { ...old.data, items: [new_item, ...old.data.items] },
              },
      );
    },
 ...
 */
export function initDeleteMutation<M extends CRUDDABLE_NAMES>({
  api,
  model,
  id,
}: {
