...
interface QUERY_KEYS_RETURN<M extends GETTABLE_NAMES> {
  get: (
    filters?: Partial<GettableTypes<M>["filters"]>,
  ) => [string, Partial<GettableTypes<M>["filters"]> | undefined];
 ...
 */
export const QUERY_KEYS = <M extends GETTABLE_NAMES>(
  model: M,
): QUERY_KEYS_RETURN<M> => ({
  /**
   ...
   */
  get: (filters?: Partial<GettableTypes<M>["filters"]>) => [
    GETTABLE_DATA[model],
    filters,
  ],
  /**
