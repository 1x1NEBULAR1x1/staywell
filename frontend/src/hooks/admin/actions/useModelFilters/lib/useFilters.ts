...
export interface UseFiltersOptions<
  B extends BaseFiltersOptions<{ id: string; [key: string]: unknown }>,
> {
  default_filters?: B;
...
>({
  ...
  // Filter state
  const [filters, setFilters] = useState<B>(
    Object.assign(
      { skip: 0, take: 10 },
      default_filters,
      memoized_permanent_fields,
    ),
  );
  ...
  const setFiltersCallback = (new_filters: Partial<B>): void => {
    setFilters((prev) => {
      ...
      if (new_filters.take !== undefined && new_filters.take !== prev.take) {
        const current_page =
          Math.floor((prev.skip || 0) / (prev.take || 10)) + 1;
        ...
        const has_filter_changed = Object.keys(new_filters).some(
          (key) =>
            key !== "take" &&
            key !== "skip" &&
            key in prev &&
